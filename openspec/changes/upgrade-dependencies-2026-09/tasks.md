...
- [ ] 5.2 Re-verify the upstream `UnleashConfigAndAccessProviderTest` / `AbstractUnleash2ProviderTest` against a real Unleash endpoint (adminBaseUrl + apiToken required) and verify the test is **not** marked local/`@Disabled`; only promote 12.3.0 if that endpoint re-verification is green, otherwise leave the spike on the branch and record the outcome. **Outcome 2026-09-11: NOT SATISFIABLE — the gate cannot be met from anything this build has, so 12.3.0 is deliberately NOT promoted.** Two independent reasons, both verified from bytes rather than assumed: (1) the named tests are **not obtainable** — `unleash-0.1.3-alpha.jar` ships 18 entries / 5 `.class` files and **0** test entries, no `-tests` classifier is published (only main + `sources`), and this repo's own unleash tests are just `FakeUnleashProviderTest` + `UnleashAutoConfigurationTest`, neither of which references `adminBaseUrl`/`apiToken`; they exist only in the upstream provider's own repository. (2) Running them would additionally need a live Unleash endpoint plus `adminBaseUrl`/`apiToken`, which are not available here. Per Decision 5 this gate is non-negotiable, so the pin stays at 11.2.1 and the spike is left on the branch. To close it: fetch the two test sources from the upstream provider repo and run them against a real Unleash instance with credentials.
