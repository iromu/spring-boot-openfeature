...
- [x] **Full-gate coverage caveat — RESOLVED.** An earlier `-Psonar` run halted at flagsmith (module 9/17) and left
      ...
      `BUILD SUCCESS`, `Total time: 03:52`, **17 modules, 81 tests, 0 failures, 0 errors, 0 skipped, 0 Checkstyle
...
> **Aggregate-total correction (2026-09-14, review cycle).** The "160 tests" figures recorded across this file are a
> **double-count**: they were produced by summing Surefire's *per-class* `Tests run:` lines **and** its *per-module*
> `Tests run:` summary lines, so every test was counted twice (160 = 2×80 before the endpoint IT was added; a later
> console tally reported 162 = 2×81 after it). The authoritative per-suite count read from
> `*/target/surefire-reports/TEST-*.xml` is **81 `@Test` methods across 22 classes** (80 before
> `UnleashEndpointIntegrationTest`), with zero `@ParameterizedTest`/`@TestFactory`/`@Nested`/`@Suite` and no Surefire
> rerun config to inflate it. Per-module figures elsewhere in this file (e.g. unleash `13`) are single-counted and
> correct. The "2 skipped" in a reproduction is the same double-sum artifact: the only genuinely skippable tests are
> the endpoint IT (`Assumptions`, when no endpoint answers) and `JsonlogicAutoConfigurationTest:80`
> `@DisabledIf("ci")`, which fires only under a `ci` property/profile that neither this gate nor `pull_request.yml`
> sets — so a no-Docker full-gate run reports **1** skip (the endpoint IT) and an endpoint-up run reports **0**.
